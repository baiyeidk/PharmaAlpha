...
```
单次请求 token：
- Input：~ 8k-15k tokens（system_prompt 1.5k + memory 1k + RAG 1.5k + chat_history 2k + tool_results 累计 5-10k）
- Output：~ 2k-4k tokens（plan 0.5k + execute summaries 1k + check 0.3k + synthesize 1-2k）
- 总计：~ 10k-20k tokens / 请求

按 DeepSeek-Chat 价格（input ~ ¥0.002/1k，output ~ ¥0.008/1k）：
- 单次请求成本：~ ¥0.04-0.08
```
